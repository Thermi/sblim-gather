/*
 * $Id: metricXen.c,v 1.1 2006/02/22 14:11:59 mihajlov Exp $
 *
...
#ifdef DEBUG
    fprintf(stderr,"--- %s(%i) : Sampling for metric ExternalViewTotalCPUTimePercentage %d\n",
...
int metricRetrTotalCPUTime( int mid, 
			    MetricReturner mret ) { 
  MetricValue * mv  = NULL;  
  ...
  if (mret==NULL) { 
	...
	*(unsigned long long*)mv->mvData 
	  = htonll((unsigned long long)(xen_statistics.cpu_time[i] * 1000));

#ifdef DEBUG
    fprintf(stderr,"--- %s(%i) : metric_id %d metric_value %f \n",
...
int metricRetrActiveVirtualProcessors( int mid, 
			    MetricReturner mret ) { 
  MetricValue * mv  = NULL;  

#ifdef DEBUG
  fprintf(stderr,"--- %s(%i) : Retrieving Xen ActiveVirtualProcessors metric\n",
	  __FILE__,__LINE__);
#endif
...
#ifdef DEBUG
    fprintf(stderr,"--- %s(%i) : Sampling for metric ActiveVirtualProcessors %d\n",
	    __FILE__,__LINE__,mid);
#endif
...
/* ---------------------------------------------------------------------------*/
int metricRetrInternalMemory( int mid, 
			      MetricReturner mret ) { 
  MetricValue * mv  = NULL;  

#ifdef DEBUG
  fprintf(stderr,"--- %s(%i) : Retrieving Xen MaximumPartitionClaimedMemory metric\n",
	  __FILE__,__LINE__);
#endif
  ...
  if (mret==NULL) { 
	...
	sprintf(buf, 
		...
		xen_statistics.max_memory[i],
		total_memory);
	size = strlen(buf);
#ifdef DEBUG
	fprintf(stderr,"%s internal memory metric: %s size: %d\n",
		xen_statistics.domain_name[i], 
		buf, size);
#endif
	mv = calloc(1, sizeof(MetricValue) +
		    size + 
...
int metricRetrHostFreePhysicalMemory( int mid, 
				      MetricReturner mret ) { 
  MetricValue * mv  = NULL;  
  
#ifdef DEBUG
  fprintf(stderr,"--- %s(%i) : Retrieving Xen HostFreePhysicalMemory metric\n",
	  __FILE__,__LINE__);
#endif
  ...
  if (mret==NULL) { 
	...
	+ sizeof(unsigned long long);
      ...
      // tbd: strcat unique domain ID as well ?!
      mret(mv);
    } 
    return 1;
  }
...
int parseXm() {
  ...
  while( NULL != (xm_result = strstr(xm_result, "(domain") ) ) {
    ...
    fprintf(stderr,"--- %s(%i) : memory: %lld\n",
	    __FILE__,__LINE__, xen_statistics.claimed_memory[num_domains]);
#endif
    ...
    fprintf(stderr,"--- %s(%i) : maxmem: %lld\n",
	    __FILE__,__LINE__, xen_statistics.max_memory[num_domains]);
#endif
    ...
    xen_statistics.domain_name[num_domains] = calloc(xm_endp-xm_result+2,0);
    strncpy( xen_statistics.domain_name[num_domains],
	     xm_result,
	     xm_endp - xm_result );
#ifdef DEBUG
    fprintf(stderr,"--- %s(%i) : domain_name: %s\n",
	    __FILE__,__LINE__, xen_statistics.domain_name[num_domains]);
#endif
