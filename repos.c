	  ...
	  if (useIntervals) {
	    ...
	    if (mv[j][0].mvTimeStamp == mv[j][1].mvTimeStamp) {
	      ...
	      m_log(M_ERROR, M_SHOW, "1: sys=%s res=%s value=%lu at %lu\n",
	          ...
	          mv[j][1].mvTimeStamp);
	      memset(vs->vsValues[actnum].viValue, 0,
